...
pygame.init()
white=(255,255,255)                             # different colours
black=(0,0,0)
red=(255,0,0)
yellow=(200,200,0)
purple = (111,25,120)
...
def message(msg,colour,i,j,size,gameDisplay,Font,state):    # funtion to display a message
    screen_text=Font.render(msg,True,colour)
    if flag==0 or state==2 or state==1:             # for initial and final display
...
def display(size,puzzle,state):
    font=pygame.font.SysFont(None,35)         # for different fonts
    ...
    if flag==0 :               # for initial display
        ...
        return flag
    if state ==0:
        moves+=1      # to mark the number of moves.
    ...
    flag=1
    i=1
    while i<(size*100+1 if size<7 else size*50+1):     # to display the puzzle in matrix form
        j=1
    ...
    for i in range(size):        # to mark zero as black block
        for j in range(size):
            k=puzzle[j][i]
            if k=='0':
                if size<=6:
                ...
                message(str(k),black,i,j,size,gameDisplay,font,0)       # to show the numbers on the blocks.
    if state ==2:                           # for final display when puzzle is solved
        if size<=6:
    ...
    elif state ==1:                                         # for final display when puzzle is not solvable
        if size<=6:
    ...
    for event in pygame.event.get():
            if event.type==pygame.QUIT:
                pygame.quit()
                quit()
            if event.type == pygame.KEYDOWN:     # to pause
                if event.key == pygame.K_p:
                    pause()
                    pygame.display.update()

                if event.key == pygame.K_f and (count==0 or count==-1 or count ==-2):  # to make the puzzle fast
                    speed = 200*3/size
                    count=1
                    pygame.display.update()
                ...
                if event.key == pygame.K_n:                     # to make the puzzle speed normal
                    speed = 500*3/size
                    count=0
                    pygame.display.update()

                if event.key == pygame.K_s and (count==0 or count==1 or count==2):    # to make the puzzle slow
                    speed = 1000*3/size
                    count=-1
                    pygame.display.update()
...
def exit(size,outcome,puzzle):       # called at the last when all moves are completed
    time.sleep(size)
    while True:
